...
public class ApuestaCombinadaGUI extends JFrame {
	private TiketApuestaGUI tiket;
	...
	private JButton btnAnadirApuesta=new JButton("Añadir Apuesta");
	// Code for JCalendar
	...
	private ArrayList <Apuesta> listaApuestasTemp;

	public ApuestaCombinadaGUI(User u)
	{
		
		this.user=u;
		this.listaApuestasTemp=new ArrayList <Apuesta>();
		textFieldCantidadApostada.setText(""); 
	...
	private void jbInit() throws Exception
	{
		...
		this.setSize(new Dimension(666, 500));
		this.setTitle("Apuesta Combinada"); //$NON-NLS-1$ //$NON-NLS-2$
		...
		btnApostar.setBounds(516, 399, 122, 23);
		getContentPane().add(btnApostar);
		
		
		btnAnadirApuesta.setBounds(491, 295, 163, 23);
		getContentPane().add(btnAnadirApuesta);
		
		btnAnadirApuesta.addActionListener(new ActionListener() {
	...
	private void jButton3_actionPerformed(ActionEvent e) {
		...
		if (user==null) {
			...
			listaApuestasTemp=tiket.apuestas;
			for ( Apuesta ap: listaApuestasTemp ) {
				cant += ap.getCantidadapostada();
			}
			if ( cant <= user.getDinero()) {
				BLFacade facade=PanelAdminGUI.getBusinessLogic();	
				
				facade.meterApuestaCombinada(user, listaApuestasTemp);
				
				System.out.println("dineroApostado"+cant);
				user = facade.addDinero(user.getId(),-cant);
				tiket.dispose();
				tiket=null;
						...
						if(check) {
							...
							if(inputPrice<=user.getDinero()) {
								if(inputPrice>selectedquestion.getBetMinimum()) {
									...
									Apuesta app = new Apuesta(-1,user, selectedquestion,comboBoxPronostico.getSelectedIndex(),inputPrice,selectedquestion.getEvent());
									listaApuestasTemp.add(app);
									if(tiket==null) {
										tiket=new TiketApuestaGUI(listaApuestasTemp);
										tiket.setVisible(true);
									}
									tiket.mov.add(0, selectedquestion.getQuestion()+" -> "+selectedquestion.getPronosticos().get(comboBoxPronostico.getSelectedIndex()).getPronostico()+"  "+inputPrice +" €");
									
									
									System.out.println(listaApuestasTemp);
